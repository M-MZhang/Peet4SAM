...
class ImageEncoderViT_task(nn.Module):
    def __init__(
        ...
        window_size: int = 0,
        global_attn_indexes: Tuple[int, ...] = (),
        adapter_hidden_dim: int= 64, # ? need to think
    ) -> None:
        """
        Args:
            img_size (int): Input image size.
        ...
        self.adapter = Task_adapter(embed_dim, adapter_hidden_dim, embed_dim, len(global_attn_indexes))
...
class Task_adapter(nn.Module):
    ...
    def __init__(
        ...
        self.adapter_mlp_list = nn.ModuleList()
        for i in range(self.num_layers):
            self.adapter_mlp_list.append(nn.Sequential(
